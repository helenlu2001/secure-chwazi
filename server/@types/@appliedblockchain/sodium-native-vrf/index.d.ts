...
declare module "@appliedblockchain/sodium-native-vrf" {
    ...
    // prove puts the proof into the buffer "proof", from the secret key and message.
    export function crypto_vrf_prove(proof: Buffer, sk: Buffer, message: Buffer): void;
    // verify puts the output hash into "output", using the public key, proof, and input message.
    // It returns 0 on success and nonzero on failure.
    export function crypto_vrf_verify(output: Buffer, pk: Buffer, proof: Buffer, message: Buffer): void;
    // keypair sets the secret key to a random seed concatenated with the public key.
    export function crypto_vrf_keypair(pk: Buffer, sk: Buffer): void;
    export function crypto_vrf_sk_to_pk(pk: Buffer, skpk: Buffer): void;
    export function crypto_vrf_sk_to_seed(seed: Buffer, skpk: Buffer): void;
    export function crypto_vrf_proof_to_hash(output: Buffer, proof: Buffer): boid;
}